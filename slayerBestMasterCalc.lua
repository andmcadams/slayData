...
function p.main( args )
	...
	slayerMasters = Slayerlib.get_masters_that_assign(desiredTask)
	
	-- Get the entire task table of the chosen slayer master.
	local taskPercents = {}
	for _, slayerMaster in ipairs(slayerMasters) do
		...
		taskPercents[slayerMaster] = p.calculate_percent(effectiveTable, desiredTask)
	end
	-- Render the results
	return p.render_table(taskPercents)
end
...
function p.calculate_percent(effectiveTable, desiredTask)
	...
	local id = Slayerlib.get_monster_id(desiredTask)
	for k, v in pairs(effectiveTable) do
		if k == id then
